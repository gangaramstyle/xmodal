...
def run_ablation(E, cfg, cache, val_cache, data_root):
    ...
    m = best
    row = dict(ckpt=c.get("ckpt", "default"), enc_width=c.get("enc_width", 768), warmstart=c["warmstart"],
               n_patients=len(tr_pids), n_tumor=c["n_tumor"], n_neg=c["n_neg"], sampling=c["sampling"],
               n_src=c["n_src"], epochs=c["epochs"], epochs2=c["epochs2"], unfreeze=c["unfreeze"],
               qsize=c["qsize"], seed=c["seed"], n_test=len(te), **m)
